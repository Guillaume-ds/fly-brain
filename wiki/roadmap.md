...
| Brain inspector — per-fly "why did it act" panel + live decision trace + real neuron graph | **all three tiers done, tested, verified live** (`decisions.md` #46, #47) — click a fly, see which tier (escape/wander/plasticity) produced its last action and the real numbers behind that decision, updating every tick; compare flies by switching which one is watched (live flies only, no generational history — explicitly scoped that way); a live-animated view of the real connectome subgraph the fly's circuits actually run on (escape and plasticity as separate views, column layout by real functional group, live spike/voltage animation) |
...
- ~~Brain inspector, tier 3 — a real neuron-graph visualization~~ done,
  see `decisions.md` #47. Live-animated view of the actual connectome
  subgraph a watched fly's circuits run on: real MaleCNS body ids/
  synapse weights (`Colony.brain_topology()`, colony-wide, sent once
  per connection) plus live per-neuron voltage/spike state
  (`Colony.neuron_state()`, streamed per tick for the one watched fly
  only). Escape and plasticity shown as separate views (they never
  interact directly, freeze+override only compares their *outputs*,
  `decisions.md` #5) with a fixed column layout by real functional
  group (KC → MBON approach/avoid → PAM/PPL DAN for plasticity; seed →
  other → motor for escape) rather than a force-directed layout, which
  would mostly produce a hairball at plasticity's ~481-neuron scale.
  `frontend/src/components/BrainGraph.tsx` renders it on a plain
  `<canvas>`. A real legibility bug (the ~300-neuron PAM group
  collapsing into a solid bar at naive fixed spacing) was caught live
  and fixed with deterministic per-group jitter. Not done: excitatory/
  inhibitory edge coloring (the sign lives in a template field not yet
  part of the wire contract — an honest simplification, not a gap
  found late).
